  ...
  <b-modal :id="refName" scrollable centered hide-footer>
    ...
    <div class="row">
      <CustomSelectBox
        ...
        :value="model.educationalCourse.activationTypeToken"
        :options="activationTypesTokensOptions"
        v-on:changeValue="model.educationalCourse.activationTypeToken = $event"
  ...
  methods: {
    async changeActivationType() {
      let formData = objectToFormData({
        ...
        activationTypeToken: this.educationalCourse.activationTypeToken,
      });
      const response = await apiEducationalCourse.changeActivationType(
        formData
      );
