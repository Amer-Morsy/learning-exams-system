  ...
  <b-modal :id="refName" scrollable centered hide-footer>
    ...
    <div class="row">
      <CustomSelectBox
        ...
        :value="model.educationalCategory.activationTypeToken"
        :options="activationTypesTokensOptions"
        v-on:changeValue="model.educationalCategory.activationTypeToken = $event"
  ...
  methods: {
    async changeActivationType() {
      let formData = objectToFormData({
        ...
        activationTypeToken: this.educationalCategory.activationTypeToken,
      });
      const response = await apiEducationalCategory.changeActivationType(
        formData
      );
