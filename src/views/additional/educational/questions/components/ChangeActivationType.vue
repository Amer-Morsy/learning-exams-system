  ...
  <b-modal :id="refName" scrollable centered hide-footer>
    ...
    <div class="row">
      <CustomSelectBox
        ...
        :value="model.question.activationTypeToken"
        :options="activationTypesTokensOptions"
        v-on:changeValue="model.question.activationTypeToken = $event"
  ...
  methods: {
    async changeActivationType() {
      let formData = objectToFormData({
        ...
        activationTypeToken: this.question.activationTypeToken,
      });
      const response = await apiQuestion.changeActivationType(formData);
