  ...
  <b-modal :id="refName" scrollable centered hide-footer>
    ...
    <div class="row">
      <CustomSelectBox
        ...
        :value="model.questionBankTitle.activationTypeToken"
        :options="activationTypesTokensOptions"
        v-on:changeValue="model.questionBankTitle.activationTypeToken = $event"
  ...
  methods: {
    async changeActivationType() {
      let formData = objectToFormData({
        ...
        activationTypeToken: this.questionBankTitle.activationTypeToken,
      });
      const response = await apiQuestionBankTitle.changeActivationType(
        formData
      );
