<template>
  <b-modal :id="refName" scrollable centered hide-footer>
    ...
    <h4 class="text-center">
      (
      <span class="co-red">
        {{ questionBank.questionBankNameCurrent }}
      </span>
      )
    </h4>
    ...
    <div class="row">
      <CustomSelectBox
        ...
        :value="model.questionBank.activationTypeToken"
        :options="activationTypesTokensOptions"
        v-on:changeValue="model.questionBank.activationTypeToken = $event"
  ...
  methods: {
    async changeActivationType() {
      let formData = objectToFormData({
        ...
        activationTypeToken: this.questionBank.activationTypeToken,
      });
      const response = await apiQuestionBank.changeActivationType(formData);
